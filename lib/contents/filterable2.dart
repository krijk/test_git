...
import 'package:faker/src/lorem.dart' show Lorem;
...
class FilterableTreeView2 extends StatefulWidget {
  const FilterableTreeView2({super.key});

  @override
  State<FilterableTreeView2> createState() => _FilterableTreeView2State();
}

class _FilterableTreeView2State extends State<FilterableTreeView2> {
  ...
  Iterable<Node> getChildren(Node node) {
    if (filter case final TreeSearchResult<Node> filter?) {
      ...
      child: Row(
        children: <Widget>[
          ExpandIcon(
            ...
            onPressed: (_) => TreeViewScope.of<Node>(context)..controller.toggleExpansion(widget.entry.node),
          ),
          if (shouldShowBadge)
            Padding(

...
import '/globals.dart';
import 'items/item_update_info.dart';

const String pageTitle = 'Update Information';
...
enum kCategory {
  ...
  static kCategory getEnum(String label) {
    ...
    return kCategory.updateInfo;
  }
}

class SettingUpdateInfo extends StatefulWidget {
  const SettingUpdateInfo({super.key});
  @override
  State<SettingUpdateInfo> createState() => SettingUpdateInfoState();
}

class SettingUpdateInfoState extends State<SettingUpdateInfo> {
  kCategory selected = kCategory.updateInfo;
  ...
  List<AbstractSettingsSection> settingContents(BuildContext context) {
    ...
    for (final kCategory e in kCategory.values) {
      list.add(getSettingSection(context, e, withTitle: false));
    }
    return list;
  }
  ...
  List<AbstractSettingsTile> getTiles(BuildContext context, kCategory category) {
    switch (category) {
      case kCategory.updateInfo:
        return <AbstractSettingsTile>[const ItemUpdateInfo()];
    }
  }
}
